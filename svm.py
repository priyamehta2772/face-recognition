...
for subject in range(1, 16):
    ...
    for test_image_name in images_list[split:]:
        ...
        target[target_index] = int(test_image_name[7:9])
        target_index += 1
        ...
        for (name, (i,j)) in face_utils.FACIAL_LANDMARKS_IDXS.items():
            index = i
            for (x, y) in shape[i:j]:
                test_distances[index] = euclidean_distance(representative_landmark, (x,y))
                index += 1
        Test[z,:] = test_distances
        z += 1
...
print("Accuracy is ",accuracy_score(target, y_pred))
